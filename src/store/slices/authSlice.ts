...
export const googleLoginAsync = createAsyncThunk<FirebaseAuthTypes.User, void>(
  'auth/googleLogin',
  async (_, { rejectWithValue }) => {
    try {
      await GoogleSignin.hasPlayServices();
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      //   state.error = null;
      // })
      // .addCase(googleLoginAsync.fulfilled, (state, action: PayloadAction<FirebaseAuthTypes.User>) => {
      //   state.loading = false;
      //   state.user = action.payload;
      //   state.isAuthenticated = true;
      // })
      ...
      .addCase(googleLoginAsync.rejected, (state, action: PayloadAction<any>) => {
        state.loading = false;
